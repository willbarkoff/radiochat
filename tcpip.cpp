...
#define MESSAGE_PARAM = "message=%s"
#define HTTP_RESPONSE_REDIRECT ""
#define HTTP_BODY_DELINIATOR "\r\n\r\n"
#define BACKUP_HTTP_BODY_DELINIATOR "\n\n"

#define SEND_MESSAGE_PATH "/send_message"

RequestHandlerFunction handle_request;
...
err_t tcp_server_recv(void *arg, struct tcp_pcb *pcb, struct pbuf *p, err_t err) {
    ...
    if (p->tot_len > 0) {
        ...
        if (strncmp(HTTP_GET, con_state->headers, sizeof(HTTP_GET) - 1) == 0) {
            ...
            is_post = 1;
            request = con_state->headers + sizeof(HTTP_POST);
            printf("Request:%s\n", con_state->headers);
            params = strstr(request, HTTP_BODY_DELINIATOR) + sizeof(HTTP_BODY_DELINIATOR) - 1;
            if (params == NULL) {
                printf("We're at the backup\n");
                params = strstr(request, BACKUP_HTTP_BODY_DELINIATOR) + sizeof(BACKUP_HTTP_BODY_DELINIATOR) - 1;
            }
            printf("Pared params:%s\n", params);
        }

        // Generate content
        con_state->result_len = handle_request(is_post, request, params, con_state->result, sizeof(con_state->result));
...
void spawn_server(RequestHandlerFunction handler) {
